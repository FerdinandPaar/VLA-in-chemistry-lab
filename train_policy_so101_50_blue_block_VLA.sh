...
# Create environment in home directory to avoid disk space issues in /tmp
export ENV_PATH="$HOME/smolvla_env_final"
...
export TMPDIR="$HOME/tmp_cache_smolvla"
mkdir -p "$TMPDIR"
export HF_HOME="$HOME/hf_cache"
export HF_HUB_ENABLE_HF_TRANSFER=1

# --- 4. Run Training ---
echo "Starting training for policy_so101_50_blue_block_VLA..."
...
python src/lerobot/scripts/lerobot_train.py \
  ...
  --policy.repo_id=mundgelenk/policy_so101_50_blue_block_VLA \
  --output_dir="$HOME/outputs/train/policy_so101_50_blue_block_VLA" \
  --job_name=policy_so101_50_blue_block_VLA \
  --policy.device=cuda \
  --wandb.enable=true \
